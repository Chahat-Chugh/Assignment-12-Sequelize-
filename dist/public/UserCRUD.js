...
import { findIndexByID, getCustomerIdByName, getCustomerList, getRoleList } from "./findOperations.js";
...
export class UserCRUD {
    ...
    async initialize() {
        ...
        this.users = [];
        data.forEach((ob) => {
            this.users.push(new User(ob.id, ob.firstname, ob.middlename, ob.lastname, ob.email, ob.phone, ob.address, ob.created_on, ob.modified_on, ob.Role.name, ob.Customer.name));
        });
    }
    load() {
        this.tableEle = document.createElement("table");
    ...
    async addUser() {
        ...
        submit.addEventListener("click", async () => {
            ...
            for (let i = 0; i <= 2; i++) {
                let s = tr.children[8].children[0].children[i];
                if (s.selected) {
            ...
            for (let i = 0; i <= 2; i++) {
                let s = tr.children[9].children[0].children[i];
                if (s.selected) {
